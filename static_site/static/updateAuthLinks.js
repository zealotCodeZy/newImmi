...
async function updateAuthLinks() {
  try {
    ...
    if (token && authLinks) {
      ...
      if (logoutLink) {
        logoutLink.onclick = async function (e) {
          ...
          await fetch('https://api.newimmi.vip/api/auth/logout', { method: 'POST', credentials: 'include' });
          location.reload();
        };
      }
      // 立即恢复显示
    ...
    const resp = await fetch(`https://api.newimmi.vip/api/auth/check-auth?t=${timestamp}`, {
      method: 'GET',
      credentials: 'include',
      headers: headers
    });
